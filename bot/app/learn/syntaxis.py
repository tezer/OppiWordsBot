...
async def unscramble_message(query, callback_data):
    ...
    if not isValid:
        return
    reveald = session.unscramble_revieled
    m = query.message
    new_data = list()
    new_keys = list()
    for i in range(len(session.unscramble_data)):
        ...
        if n == int(callback_data['data']):
            revield = reveald + ' ' + session.unscramble_keys[i]
            continue
        new_data.append(n)
        new_keys.append(session.unscramble_keys[i])
    await do_unscramble(session, new_keys, new_data, session.unscramble_sentence, reveald, m)



async def do_unscramble(session, keys, data, sentence, reveald, message):
    actions = ['unscramble'] * len(data)
    k = bot_utils.to_vertical_keyboard(keys, data, actions)
    await message.edit_text("*" + sentence[1] + "*" + "\n" + reveald, reply_markup=k)
    session.unscramble_keys = keys
    session.unscramble_data = data
    session.unscramble_revieled = reveald
